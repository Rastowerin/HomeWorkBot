...
while True:
    ...
    if update[0][0] == 4:
        if update[0][0] == 4 and '!дз' in update[0][6] and '!!дз' not in update[0][6] and update[0][6] != '!дз':
            ...
            print(subject)
            if '%s' % subject in function.homework:
                function.homework['%s' % subject] = update[0][6].split(': ')[2]
                print(function.homework['%s' % subject])
        print(update)
        user_id = update[0][3]
        user_name = function.vk_bot.method('users.get', {'user_ids': user_id})
        if 'привет' in update[0][6]:
            function.write_msg(user_id, 'здоров, ' + (user_name[0]['first_name']))  # cooбщение пользователю
